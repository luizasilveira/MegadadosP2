...
if __name__ == '__main__':
    ...
    sc = pyspark.SparkContext(appName="Projeto2_small")
    rdd = sc.sequenceFile("s3://megadados-alunos/web-brasil_small")
    ...
    inter_csv = inter_df.to_csv(
        "s3://megadados-alunos/gabriela-luiza/inter_small.csv", index=False)
    iphone_df = pd.DataFrame(list_iphone_rel, columns=[
                             "palavra", "relevancia"])
    iphone_csv = iphone_df.to_csv(
        "s3://megadados-alunos/gabriela-luiza/iphone_small.csv", index=False)
    android_df = pd.DataFrame(list_android_rel, columns=[
                              "palavra", "relevancia"])
    android_csv = android_df.to_csv(
        "s3://megadados-alunos/gabriela-luiza/android_small.csv", index=False)
